    ...
    private void Update()
    {
        if (!PauseMenu.Instance.IsPaused)
        {
            ...
            if (Player != null)
            {
                ...
                {
                    Vector2 InputAxes = ((Player.position - this.transform.position).normalized);
                    Movement.Set(InputAxes);
                    ...
                    if (WeaponShotTimer <= 0f && PlayerRaycast.distance <= AttackRange && (ViewportLocation.x == Mathf.Clamp01(ViewportLocation.x) && ViewportLocation.y == Mathf.Clamp01(ViewportLocation.y)))
                    {
                        ...
                        if (Hit)
                        {
                            ...
                            if (Hit.transform.gameObject.TryGetComponent<Crate>(out Crate Crate)) {
                                Crate.Damage((BaseDamage * Mathf.Clamp(((int.Parse((Settings.Get("Difficulty", 1).ToString()))) + 1), 1, int.MaxValue)));
                            }
                        ...
                        Vector2 RecoilDirection = Rigidbody.transform.position - FirePoint.position;
                        RecoilDirection.Normalize();

                        Vector2 RecoilAcceleration = ((RecoilDirection * RecoilForce) / Rigidbody.mass);
                        Vector2 RecoilVelocity = RecoilAcceleration * Time.fixedDeltaTime;
                        Rigidbody.velocity += RecoilVelocity;
